# AI 生成器平台 — 开发文档 & 架构方案 (v0.5)

> 版本:v0.5(草稿,持续共创)
> 最后更新:2026-07-17
> 一句话定位:一个通过 AI 对话即可生成**网站/前端页面**(后续扩展文档、代码)的平台;支持多模型自选、多 agent 编排、用户系统(账号密码/手机短信)、RAG 增强,可自托管、可横向扩容,并预留收费模式扩展点。
...
| 模块 | 说明 |
|------|------|
| 用户系统 | 账号密码注册/登录 + 手机号短信验证码注册/登录/找回 |
...
| 短信 | **SMSService 抽象层**(阿里云/腾讯云/Mock) | 同模型抽象层思路,见 §4.4 |
...
### 4.1 需求
- 支持**账号密码**注册/登录。
- 支持**手机号 + 短信验证码**注册/登录/找回密码。
- 设计为**未来可平滑接入收费模式**:用户带「套餐/层级(plan)」概念,配额由套餐决定;计费/订阅相关表预留扩展位。
...
### 4.3 关键流程
- 注册(账号密码):提交 username/email + password → 哈希入库 → 签发 JWT。
- 注册/登录(手机):`POST /sms/send`(手机号)→ Redis 存 code(5min)→ `POST /auth/phone-login`(校验 code)→ 签发 JWT。
- 密码找回:手机短信发 code → 重设密码。
- 限流:注册/发码接口用 Redis 令牌桶限频(同 §9 限流机制),防短信轰炸与 Key 盗刷。

### 4.4 抽象层设计(SMSService)
与模型抽象层同思路:新增 Provider 即可接入不同短信商,本地开发用 Mock 不发真实短信。
```python
...
SMS_PROVIDERS: dict[str, SMSService] = {
    "mock":    MockSMSService(),          # 本地开发:打印到日志
    "aliyun":  AliyunSMSService(...),     # 阿里云短信
    "tencent": TencentSMSService(...),    # 腾讯云短信
}
```
...
- **M1 可用**:用户系统(账号密码 + 手机短信登录/找回)→ JWT 鉴权 → 用量限流(plan/tier 配额,Redis)→ 项目/对话持久化(MySQL)→ 基础 RAG(components 检索)。
- **M2 可分享 + 记忆**:分享只读链接 + memory 记忆回写与检索 + 生成队列/Worker + 迭代修改。
- **M3 增强**:Designer agent 深度美化、Sandpack 多文件、版本历史、导出 zip、模板库运营、收费模式接入、多服务器扩容上线。

---

## 12. ⬜ 开放问题汇总(待拍板)

1. **短信服务商**:阿里云短信 / 腾讯云短信 / 先用 Mock 占位?(接真实短信需签名+模板审核,影响 M1 实现)
2. JWT 细节:Access Token 时长、是否用 HttpOnly Cookie 防 XSS?
3. ORM:SQLAlchemy(推荐)还是 Prisma(可选)?
4. Embedding 模型用哪家?(§7)
5. HY3 的 API 文档/鉴权方式?(写 Provider 需要)
6. 视觉风格 / 配色参考?(影响前端 UI 起手)
7. 模型调用月度预算?(影响限流与默认模型策略)
8. 各 agent 的 system prompt 基调 / 目标用户画像?(影响生成风格)
9. 收费模式:现在就建 `Subscription`/`Payment` 表,还是只留 plan 字段 + 配额抽象(推荐)?
