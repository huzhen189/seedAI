...
- `frontend`(Next.js)
- `business-api`(业务服务:FastAPI,持 DB/JWT 配置)
...
- 长耗时生成走 Redis 队列 + 独立 Worker(M2,可挂在 AI 服务侧)。
...
```
session:<token>           会话(业务)
ratelimit:<userId>        限流计数(业务)
sms:code:<phone>          短信验证码(业务,预留)
auth:refresh:<userId>     Refresh Token(业务)
queue:generate            生成任务队列(AI,M2)
cache:models              模型列表缓存(AI)
```
...
- **M1 可用**:业务服务补全用户系统(账号密码 + JWT)→ 配额限流(plan/tier,Redis)→ 项目/对话持久化(MySQL)→ AI 服务基础 RAG(components 检索)。
- **M2 可分享 + 记忆**:分享只读链接 + memory 回写与检索 + 生成队列/Worker + 迭代修改。
