...
INTENT_SYSTEM = (
    ...
    "learn→explain(解释概念)|debug(排查报错)|compare(技术对比)|casual(闲聊)|design(UI设计配色/原型)|search(搜索查资料)\n"
    "code→snippet(写函数片段)|component(写UI组件)|fix(修Bug)|refactor(重构/评审)\n"
    "build→page(单页)|site(完整站)|modify(修改已有)|game(互动小游戏/网页游戏)\n"
    "doc→readme(README)|tutorial(教程)|plan(方案设计)\n"
    "translate→text(翻译)|code_lang(跨语言翻译)\n"
    "unsupported→无子类\n"
    f"{UNSUPPORTED_HINT}\n\n"
    "industry(13选1, build/doc 时必填, 其他填 none):\n"
