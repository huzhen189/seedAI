...
def run_rules(messages: list[dict]) -> RuleResult:
    ...
    build_kw = ["建", "开发", "搭建", "搞一个", "弄一个", "整一个",
                ...
                # v1.0: code 方向归入 build
                "代码", "函数", "脚本", "组件", "模块", "类", "class",
                "修复", "bug", "报错", "error", "fix", "调试", "traceback",
                "改一下", "改下", "重构", "评审", "review", "性能优化",
                "css", "样式", "动画", "js", "javascript", "typescript", "ts"]
    # chat 关键词(讲解/设计/搜索/闲聊/翻译/文档)
    chat_kw = ["是什么", "怎么", "如何", "为什么", "原理", "底层", "怎么实现", "如何实现",
    ...
    if any(w in t for w in chat_kw):
        keywords = [w for w in chat_kw if w in t]
        logger.info("[规则] 命中: chat 关键词=%s industry=%s conf=0.7", keywords[:5], industry)
