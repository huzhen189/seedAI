...
# v2 修正(#18/#20 漏召): 站点语义名词扩展(教育平台/旅游小程序/官网类短语/在线X平台等)。
# 路由缺口修复(简历/作品集口吻): 与 cascade.py `_site_nouns` 个人/作品类站点词保持同步,
#   新增「简历/作品集/个人主页/个人简历/简历网站/简历网页/个人作品集/简历页/portfolio」,
#   否则多意图拆分场景下「帮我做简历网站, 还要个博客」会漏认简历站点(只因 _BUILD_KW 含简历词才有 OR 兜底,
#   但站点名词表本身应自洽 —— 见 cascade.py:474 同款注释, 两表必须一致)。
_SITE_NOUNS = (
    "网站", "官网", "门户", "博客", "blog", "主页", "首页", "落地页", "landing", "h5", "H5",
    "小程序", "站点", "平台", "商城", "电商", "web", "官网页面", "web页面",
    "在线教育", "在线课程", "教育平台", "旅游平台", "旅游官网", "旅游小程序",
    "公司官网", "企业官网", "个人网站", "产品官网", "品牌官网", "官网首页",
    "官网页面", "展示官", "宣传页", "营销页", "活动页", "官网主页",
    # 路由缺口修复: 与 cascade.py `_site_nouns` 个人/作品类站点词同步(简历口吻直路由 build/site)
...
# v2 修正(#18/#15 漏召): 补充裸"并X/还要/也要"等安全多音节连词 —— 既作门控触发,
#   也作切分锚点(见 _SPLIT_BEFORE)。刻意不收录单字"并"(会与"合并"等误切分)。
_MULTI_TRIGGER_WORDS: tuple[str, ...] = (
    "并且", "而且", "同时", "再帮我", "还要", "还需", "也要", "也帮我",
    "另外", "顺带", "以及", "，然后", ",然后", "顺手", "除此之外",
    "并且帮我", "然后再", "并写", "并生成", "并搜", "并设计", "并做", "并帮我",
)
...
_SPLIT_BEFORE = (
    ...
    "先", "再", "最后", "步骤",
    # v2 修正: 裸"并X/也要/也要"等多音节连词(避免单字"并"误切分), 既作门控也作切分锚点。
    "并写", "并生成", "并搜", "并设计", "并做", "并帮我", "并开发", "并创建",
    "也要", "也要写", "还要写", "还要搜", "还需", "也帮我",
)
# 切分锚点: 前置标点=中文逗号/英文逗号/分号/句号/问号/感叹号/换行, 后接连词才插换行。
# v2 修正: 加入中文逗号"(?<=，)" —— 否则"做旅游官网，写文章，搜索..."这类逗号长句不切分(#20)。
_SPLIT_RE = re.compile(r"(?<=[\u4e00-\u9fff，,；;。！？!?\n])(" + "|".join(_SPLIT_BEFORE) + r")")
    ...
    *,
    ...
    for seg in segments:
        try:
            ...
            seg_txt = seg["text"]
            _site_noun_hit = any(kw in seg_txt for kw in _BUILD_KW) or any(
                kw in seg_txt for kw in _SITE_NOUNS)
            if _site_noun_hit and any(vb in seg_txt for vb in _SITE_BUILD_VERBS):
